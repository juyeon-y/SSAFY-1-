...
import com.ssafy.env.view.EnvView;
...
public class HomeInfoView{
	...
	/**주변 환경 오염 정보 버튼*/
	private JButton			  	envPollutionBt;
	
	/**주변 상가 정보 버튼*/
	private JButton				comAreaInfoBt;
...
//		});
	...
	public void setMain(){
		...
		comAreaInfoBt = new JButton("주변 상가 정보");
		envPollutionBt = new JButton("환경 오염 정보");
		...
		leftR.add(comAreaInfoBt);
		leftR.add(envPollutionBt);
		...
		comAreaInfoBt.addActionListener(e->{
			if(row==-1) {
			new ComInfoView();
			}
			else {
				new ComInfoView(homeModel.getValueAt(row, 1).toString());
			}
		});
		
		envPollutionBt.addActionListener(e->{
			new EnvView();
